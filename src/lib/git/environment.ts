...
import { resolveGitProxy } from '$lib/utils/resolve-git-proxy';
 ...
 * url to fall back on.
 */
export function getFallbackUrlForProxyResolve(account: IGitAccount | null, repository: Repository) {
	...
	if (account !== null) {
		...
		return account.endpoint === getDotComAPIEndpoint() ? 'https://github.com' : account.endpoint;
	}
 ...
 */
export async function envForRemoteOperation(account: IGitAccount | null, remoteUrl: string) {
	return {
