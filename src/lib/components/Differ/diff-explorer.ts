 ...
 */
export function diffHunkForIndex(hunks: ReadonlyArray<DiffHunk>, index: number): DiffHunk | null {
 ...
 */
export function diffLineForIndex(hunks: ReadonlyArray<DiffHunk>, index: number): DiffLine | null {
...
export function lineNumberForDiffLine(diffLine: DiffLine, hunks: ReadonlyArray<DiffHunk>): number {
	let lineOffset = 0;
 ...
 */
export function getLineInOriginalDiff(hunks: ReadonlyArray<DiffHunk>, index: number) {
...
export function findInteractiveDiffRange(
	...
	let rangeType: DiffRangeType | null = getNextRangeType(null, hunk.lines[relativeIndex]);
	...
	const from = contextLineBeforeIndex !== null ? contextLineBeforeIndex : hunk.unifiedDiffStart + 1;
	...
	const to = contextLineAfterIndex !== null ? contextLineAfterIndex : hunk.unifiedDiffEnd;
...
function getNextRangeType(
	...
	if (currentLine.type !== DiffLineType.Add && currentLine.type !== DiffLineType.Delete) {
	...
	if (
		(currentLine.type === DiffLineType.Add && currentRangeType !== DiffRangeType.Additions) ||
		(currentLine.type === DiffLineType.Delete && currentRangeType !== DiffRangeType.Deletions)
	) {
