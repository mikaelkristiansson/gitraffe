...
import { DefaultDiffExpansionStep } from '$lib/git/diff-parser';
import { diffLineForIndex } from './diff-explorer';
import { getFirstAndLastClassesUnified } from './diff-helper';
 ...
 * one version and we need to be resilient about that.
 */
export function getTokens(lineNumber: number | null, tokens: ITokens | undefined) {
	...
	if (tokens !== undefined && lineNumber !== null && tokens[lineNumber - 1] !== undefined) {
...
export class DiffSyntaxMode {
	...
	public constructor(hunks?: ReadonlyArray<DiffHunk>, oldTokens?: ITokens, newTokens?: ITokens) {
		this.hunks = hunks;
	...
	public token = (stream: CodeMirror.StringStream, state: IState): string | null => {
		...
		if (stream.sol()) {
			...
			const nextLineToken = typeof nextLine === 'string' ? TokenNames[nextLine[0]] : undefined;
			...
			if (token === DiffSyntaxToken.Hunk) {
				...
				if (matches !== null) {
					...
					if (
						...
						oldStartLine - state.previousHunkOldEndLine > DefaultDiffExpansionStep
					) {
		...
		const lineTokens = getTokensForDiffLine(diffLine, this.oldTokens, this.newTokens);
...
CodeMirror.defineMode(
	...
	function (config: CodeMirror.EditorConfiguration, modeOptions?: IDiffSyntaxModeOptions) {
		if (!modeOptions) {
			throw new Error('I needs me some options');
		}

		return new DiffSyntaxMode(modeOptions.hunks, modeOptions.oldTokens, modeOptions.newTokens);
	}
);
