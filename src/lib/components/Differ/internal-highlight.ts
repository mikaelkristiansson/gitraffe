...
export async function highlight(
	...
	const mode = await detectMode({ contentLines, basename, extension, tabSize, lines });

	const lineFilter = lines && lines.length ? new Set<number>(lines) : null;
	...
	return new Promise<ITokens>((resolve, reject) => {
		...
		const lines = contentLines.concat();
		...
		for (const [ix, line] of lines.entries()) {
			...
			while (!lineStream.eol()) {
				const token = readToken(mode, lineStream, state, true);
