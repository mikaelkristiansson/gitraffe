import { HiddenBidiCharsRegex, getLargestLineNumber } from '$lib/git/diff-parser';
import {
	DiffHunk,
...
function mergeDiffHunks(hunk1: DiffHunk, hunk2: DiffHunk): DiffHunk {
	...
	const newHunkLines = [newFirstHunkLine, ...allHunk1LinesButFirst, ...allHunk2LinesButFirst];
...
export function expandWholeTextDiff(
	...
	while (
		...
		(result.hunks.length === 1 && result.hunks[0].expansionType === DiffHunkExpansionType.Up)
	) {
...
export function expandTextDiffHunk(
	...
	const adjacentHunkIndex =
		isExpandingUp && hunkIndex > 0
			...
			: !isExpandingUp && hunkIndex < diff.hunks.length - 1
				? hunkIndex + 1
				: null;
	const adjacentHunk = adjacentHunkIndex !== null ? diff.hunks[adjacentHunkIndex] : null;
	...
	let [from, to] = isExpandingUp
		...
		: [newLineNumber + hunk.header.newLineCount, newLineNumber + hunk.header.newLineCount + step];
	...
	if (adjacentHunk !== null) {
		if (isExpandingUp) {
			const upLimit = adjacentHunk.header.newStartLine + adjacentHunk.header.newLineCount;
	...
	const hasHiddenBidiChars =
		diff.hasHiddenBidiChars || newLines.some((line) => HiddenBidiCharsRegex.test(line));

	// Update the resulting hunk header with the new line count
	const newHunkHeader = new DiffHunkHeader(
		isExpandingUp ? hunk.header.oldStartLine - numberOfLinesToAdd : hunk.header.oldStartLine,
		hunk.header.oldLineCount + numberOfLinesToAdd,
		isExpandingUp ? hunk.header.newStartLine - numberOfLinesToAdd : hunk.header.newStartLine,
	...
	const expansionType = getHunkHeaderExpansionType(hunkIndex, newHunkHeader, previousHunk);
	...
	const newHunkLastLine = newHunkHeader.newStartLine + newHunkHeader.newLineCount - 1;
	const followingHunks =
		newHunkLastLine >= newContentLines.length
			...
			: diff.hunks.slice(followingHunksStartIndex).map((hunk, hunkIndex) => {
					...
					const shouldComputeNewExpansionType = hunkIndex === 0 && !isLastDummyHunk;
					...
					return new DiffHunk(
						...
						shouldComputeNewExpansionType
							? getHunkHeaderExpansionType(followingHunksStartIndex, hunk.header, updatedHunk)
							: hunk.expansionType
					);
				});
...
export function getTextDiffWithBottomDummyHunk(
	...
	const lastHunkNewLine = lastHunk.header.newStartLine + lastHunk.header.newLineCount;

	if (lastHunkNewLine >= numberOfNewLines) {
		return null;
	}
	const dummyOldStartLine = lastHunk.header.oldStartLine + lastHunk.header.oldLineCount;
	const dummyNewStartLine = lastHunk.header.newStartLine + lastHunk.header.newLineCount;
