...
export function getContextByClass<
  T extends new (...args: any) => InstanceType<T>,
>(key: T): InstanceType<T> {
...
export function getContextStoreByClass<
  T extends new (...args: any) => InstanceType<T>,
>(key: T): Readable<InstanceType<T>> {
  const instance = svelteGetContext<Readable<InstanceType<T>> | undefined>(key);
  if (!instance)
    throw new Error(`no instance of \`Readable<${key.name}>\` in context`);
  return instance;
}
